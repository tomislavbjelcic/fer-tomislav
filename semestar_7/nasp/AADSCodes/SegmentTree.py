...
class Interval:
    ...
    def intersection(self,Iother):
        if type(Iother) is Interval:
            if Iother.v1>self.v2 or Iother.v2<self.v1: return None
            return Interval(True,max(self.v1,Iother.v1),min(self.v2,Iother.v2),True)
        else: return None
...
class VerticalTree(AVLTree):
    ...
    def query(self,HIq,VIq):
        ...
        if nsplit.L is None and nsplit.R is None:
...
class SegmentTree:
    ...
    def _insertSegment(self,n,ls):
        if n.I.containedIn(ls):
            ...
            if n.leftChild is not None:
                nl=n.leftChild
                if nl.I.intersectedBy(ls)>0: self._insertSegment(nl,ls)
            if n.rightChild is not None:
                nr=n.rightChild
                if nr.I.intersectedBy(ls)>0: self._insertSegment(nr,ls)
    def _create(self,ei):
        lei=len(ei)
    ...
    def _SQFindSplittingNode(self,Iq):
        ...
        while n.leftChild is not None or n.rightChild is not None:
            ...
            lci,rci=lc.I.intersectedBy(Iq),rc.I.intersectedBy(Iq)
            if lci==1 and rci==1:
                return n
            if lci==2: n=lc
            else: n=rc
