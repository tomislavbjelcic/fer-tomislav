...
def OneDRangeQuery(tau,axis,mu1,mu2):
    ...
    if isinstance(nsplit,Value):
        if nsplit.isInRangeAxis(axis,mu1,mu2): rv.append(nsplit)
    else:
        n=nsplit.leftChild
        while n is not None and not isinstance(n,Value):
            if mu1 is not None and mu1<=n.value[axis]:
                ReportSubtree(rv,n.rightChild)
                n=n.leftChild
            else: n=n.rightChild
        ...
        n=nsplit.rightChild
        while n is not None and not isinstance(n,Value):
            if mu2 is not None and n.value[axis]<=mu2:
                ReportSubtree(rv,n.leftChild)
                n=n.rightChild
            else: n=n.leftChild
        if n is not None and isinstance(n,Value) and n.isInRangeAxis(axis,mu1,mu2):
...
class Value(Node):
    ...
    def isInRangeAxis(self,axis,mu1,mu2):
        for v in self.value:
            if mu1 is not None and v[axis]<mu1: return False
            if mu2 is not None and v[axis]>mu2: return False
        return True
