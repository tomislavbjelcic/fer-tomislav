...
Postoji niz strategija za odabir početnih $K$ centroida. Prva mogućnost jest kao početne centroide postaviti $K$ slučajno izabranih točaka iz $\mathcal{D}$. Slučajan odabir nije baš najbolja ideja, jer rezultati neće biti dobri ako neki od početnih $K$ centroida se nalaze unutar istih prirodnih grupa.\\
...
$\left(\mathbf{x} - \boldsymbol{\mu}\right)^{\mathrm{T}}
        \boldsymbol{\Sigma}^{-1}
        \left(\mathbf{x} - \boldsymbol{\mu}\right)
$ je kvadrat takozvane \textbf{Mahalanobisove udaljenosti} između vektora $\mathbf{x}$ i lokacije $\boldsymbol{\mu}$. To je višedimenzijska generaliziracija ideje udaljenosti točke od srednje vrijednosti u broju standardnih devijacija kod jednodimenzijskih razdioba.
...
\end{itemize}
Odluka kada će postupak završiti je također parametar algoritma. Ukoliko se želi zaustaviti postupak nakon dostignutih $K$ grupa, broj $K$ je potrebno predati kao parametar, a ako se ne želi premašiti najmanja udaljenost $\epsilon$ između grupa tijekom spajanja, tada je potrebno specificirati broj $\epsilon$.\\
\begin{croatianalgorithm}[H]
...
Opisani algoritmi su pokrenuti, vrednovani i uspoređeni na 6 različita, relativno jednostavna skupa dvodimenzijskih podataka sa 
realnim značajkama.

\kljucnerijeci{Grupiranje, nenadzirano strojno učenje, mjera udaljenosti, mjera sličnosti, Euklidova udaljenost, K-sredina, centroid, K-means++, metoda koljena, model miješane gustoće, GMM, MLE, EM algoritam, AIC, hijerarhijsko aglomerativno grupiranje, kriterij spajanja, Ward, model gustoće točaka, DBSCAN, šum, Davies-Bouldin indeks, vrijednost siluete, Randov indeks, uzajamna informacija, Scikit-Learn, Python}
\end{sazetak}
...
K-means requires knowledge of the number of clusters and is able to group convex group shapes.
Hierarchical agglomerative clustering is based on linkage criterion, builds a hierarchy of clusters by merging them, and is not as efficient as K-means, but is applicable to all types of data.
DBSCAN is based on the density model which discovers and clusters dense regions of data. DBSCAN is fast, has a notion of noise and is able to cluster arbitrarily shaped clusters. EM algorithm for Gaussian mixture model aims to estimate parameters of Gaussian mixture model that views the data as realizations of several Gaussian distributions. EM algorithm is fast and is able to cluster convex groups of various shapes.
...
\keywords{Clustering, unsupervised machine learning, distance measure, similarity measure, Euclidean distance, K-means, centroid, K-means++, elbow method, mixture model, GMM, MLE, EM algorithm, AIC, hierarchical agglomerative clustering, linkage criterion, Ward, density model, DBSCAN, noise, Davies-Bouldin index, silhouette, Rand index, mutual information, Scikit-Learn, Python}
\end{abstract}
