...
\end{equation}
Uveli smo oznaku $\pi_k = P \left(y = k\right)$, i tu vjerojatnost nazivamo \textbf{koeficijent mješavine} \engl{mixture weight}. Jasno je da mora vrijediti
\[\sum_{k=1}^{K} \pi_k = 1\]
...
Parametre modela $\boldsymbol{\theta}$ procjenjujemo \textbf{metodom najveće izglednosti} \engl{maximum likelihood estimation}. Definirajmo funkciju izglednosti parametara $\boldsymbol{\theta}$ na uzorku $\mathcal{D}$. Pretpostavimo da su primjeri iz uzorka nezavisni, tada možemo funkciju izglednosti definirati kao umnožak gustoća svakog primjera iz uzorka:
...
\end{algorithmic}
\end{croatianalgorithm}
Iz ‘‘natreniranih’’ parametara modela lako možemo izračunati odgovornosti iz formule \ref{odgovornost}, što je rezultat mekog grupiranja:
\[
P \left(y = k \middle| \mathbf{x}^{(i)}\right) =
h_{k}^{(i)} =
\frac
...
\begin{figure}[H]
    ...
    \label{fig:gmmem}
\end{figure}
Nijanse boja točaka označuju u kojoj mjeri točka pripada pojedinim grupama. Nakon svake iteracije se lokacije (zamišljene točke sa crnim obrubom) i kovarijacijske matrice (vizualizirane elipsama) mijenjaju dok se ne postigne stacionarno stanje: konvergencija log-izglednosti ili parametara modela. 
...
\begin{enumerate}
    \item \textbf{Izotropne}: svaka komponenta ima kovarijacijsku matricu oblika $\boldsymbol{\Sigma}_k = \sigma_{k}^2 \, \mathbf{I}$, gdje je $\mathbf{I}$ jedinična matrica, a $\sigma_{k}^2$ dijeljena varijanca svake komponente. Značajke svake komponente su tada nekorelirane, imaju jednaku varijancu i grupe su sferičnog oblika.
...
Kao primjer uzmimo da su podaci skup od 5 prirodnih brojeva od 1 do 5. Pretpostavimo da smo podatke grupirali na sljedeći način:
